...
void loop() {
  ...
  if ((center_line > LINE_THRESHOLD) && (left_line > LINE_THRESHOLD) && (right_line > LINE_THRESHOLD)) {
    ...
    pwm.setPWM(FRONT_LEFT_SERVO, 0, FRONT_LEFT_OFF + SERVO_SPEED / 2);
    pwm.setPWM(FRONT_RIGHT_SERVO, 0, FRONT_RIGHT_OFF - SERVO_SPEED);
    pwm.setPWM(BACK_LEFT_SERVO, 0, BACK_LEFT_OFF + SERVO_SPEED / 2);
    pwm.setPWM(BACK_RIGHT_SERVO, 0, BACK_RIGHT_OFF- SERVO_SPEED);
  }
  else if ((center_line > LINE_THRESHOLD) || (left_line < LINE_THRESHOLD) && (right_line > LINE_THRESHOLD)) {
    pwm.setPWM(FRONT_LEFT_SERVO, 0, FRONT_LEFT_OFF + SERVO_SPEED);
    pwm.setPWM(FRONT_RIGHT_SERVO, 0, FRONT_RIGHT_OFF - SERVO_SPEED / 2);
    pwm.setPWM(BACK_LEFT_SERVO, 0, BACK_LEFT_OFF + SERVO_SPEED);
    pwm.setPWM(BACK_RIGHT_SERVO, 0, BACK_RIGHT_OFF - SERVO_SPEED / 2);
  }
  else {
    pwm.setPWM(FRONT_LEFT_SERVO, 0, FRONT_LEFT_OFF);
    pwm.setPWM(FRONT_RIGHT_SERVO, 0, FRONT_RIGHT_OFF);
